...
export function MediaPicker({ 
  ...
  useEffect(() => {
    if (tagArray.length > 0 && showGlobalSearch) {
  ...
  }, [tagArray, galleryMedia, showGlobalSearch])
  ...
  const getDefaultTitle = () => {
    switch (mode) {
      case 'messaging': return 'Select Media to Send'
      case 'post': return 'Add Media to Post'
      default: return 'Add Media to Gallery'
    }
  }

  const getDefaultConfirmText = () => {
    switch (mode) {
      case 'messaging': return `Send ${selectedMedia.length} Media Item${selectedMedia.length !== 1 ? 's' : ''}`
      case 'post': return `Add ${selectedMedia.length} Media Item${selectedMedia.length !== 1 ? 's' : ''} to Post`
      default: return `Add ${selectedMedia.length} Media Item${selectedMedia.length !== 1 ? 's' : ''} to Gallery`
    }
  }

  const modalTitle = title || getDefaultTitle()
  const confirmButtonText = confirmText || getDefaultConfirmText()

  if (!isOpen) return null
  
  // Don't render if no userId is provided
  if (!userId) {
    console.error('MediaPicker: No userId provided');
    return null;
  }
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
            {activeTab === 'upload' && showUpload ? (
              ...
              <div className="space-y-4">
                ...
                {showGlobalSearchResults && tagArray.length > 0 && showGlobalSearch && (
                ...
                {/* Filter Section */}
                {showFilters && (galleryMedia.length > 0 || userGalleries.length > 0) && (
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="relative">
                          <button
                            ...
                            className={`p-2 rounded-lg transition-colors duration-200 ${
                              showFilterControls || filterTags.length > 0 || selectedGalleryId
                ...
                {showGlobalSearchResults && showGlobalSearch && (
                  <div>
                    <h3 className="text-sm font-medium text-gray-700 mb-3 flex items-center">
                      <Globe className="h-4 w-4 mr-2" />
                ...
                {galleryMedia.length === 0 && !galleryLoading && !showGlobalSearchResults && (
